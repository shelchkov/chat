...
describe('Messages component', () => {
  ...
  const useWidthMock = jest.spyOn(useWidth, "useWidth").mockReturnValue({ width: 1200, isMore: true })
  jest.spyOn(useLatestMessages, "useLatestMessages").mockReturnValue({ latestMessages: undefined, updateLatestMessage: updateLatestMessageMock })
  jest.spyOn(userUtils, "markOnlineUsers").mockReturnValue(undefined)
  jest.spyOn(userUtils, "addLatestMessagesToUsers").mockReturnValue(undefined)
  ...
  describe('when opened on desktop', () => {
    ...
    it("MessagesList receives handleNewMessage", () => {
      expect(component.find(MessagesList).prop("handleNewMessage")).toBe(updateLatestMessageMock)
    })
    ...
    describe('and friend was selected', () => {
      ...
      it("UsersList receives selectedUserId", () => {
        expect(component.find(UsersList).prop("selectedUserId")).toBe(userMock2.id)
      })

      it("MessagesList receives selectedUser", () => {
        expect(component.find(MessagesList).prop("selectedUser")).toBe(userMock2)
      })
    })
  })
  ...
  describe('when opened on mobile', () => {
    ...
    describe('and friend was selected', () => {
      ...
      it("MessagesList receives isMobile", () => {
        expect(component.find(MessagesList).prop("isMobile")).toBeTruthy()
      })
    })
  })
})
